...
# def semantic_match(req_text: str, eval_text: str, threshold=0.65):
#     req_emb = model.encode(req_text, convert_to_tensor=True)
#     eval_emb = model.encode(eval_text, convert_to_tensor=True)
#     similarity = util.pytorch_cos_sim(req_emb, eval_emb).item()
#     return similarity, similarity >= threshold
...
# def map_tests_to_checklist(eval_report: Dict, requirements: List[Dict]):
#     mappings = []
...
# def save_compliance_report(df: pd.DataFrame, csv_path: str, pdf_path: str):
#     try:
...
# if __name__ == "__main__":
#     try:
#         # 1️⃣ Read evaluation JSON from stdin
#         input_json = sys.stdin.read()
#         eval_report = json.loads(input_json)

#         # 2️⃣ Load RBI requirements from repo
#         requirements_path = os.path.join(os.path.dirname(__file__), "proxy_updated_rbi_requirements.json")
#         with open(requirements_path, 'r', encoding='utf-8') as f:
#             data = json.load(f)
#             requirements = data.get("requirements", [])

#         if not eval_report or not requirements:
#             print("❌ Error: No evaluation data or requirements found.")
#             sys.exit(1)

#         # 3️⃣ Map tests to checklist
#         df = map_tests_to_checklist(eval_report, requirements)

#         # 4️⃣ Save reports in tmp/ folder
#         tmp_dir = os.path.join(os.path.dirname(__file__), "tmp")
#         os.makedirs(tmp_dir, exist_ok=True)
#         csv_path = os.path.join(tmp_dir, "rbi_compliance_report.csv")
#         pdf_path = os.path.join(tmp_dir, "rbi_compliance_report.pdf")

#         save_compliance_report(df, csv_path, pdf_path)
